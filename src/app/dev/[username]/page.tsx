...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!dev) {
    return { title: "Developer Not Found - DevX GitHub City" };
  }

  const contribs = (dev.contributions_total && dev.contributions_total > 0) ? dev.contributions_total : dev.contributions;
  const title = `@${dev.github_login} - DevX GitHub City | ${contribs.toLocaleString()} contributions`;
  const description = `See @${dev.github_login}'s building in DevX GitHub City. ${contribs.toLocaleString()} contributions, ${dev.public_repos.toLocaleString()} repos, ${dev.total_stars.toLocaleString()} stars. Rank #${dev.rank ?? "?"} in the city.`;
